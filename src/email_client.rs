...
mod tests {
    ...
    async fn send_email_fires_a_request_to_base_url() {
        // Arrange
        let mock_server = MockServer::start().await; //receives a request, iterates over all the mounted mocks to check if the request matches their conditions
    ...
    async fn send_email_fails_if_the_server_returns_500() {
        ...
        let content: String = Paragraph(1..10).fake();
        Mock::given(any())
            .respond_with(ResponseTemplate::new(500)) // Not a 200 anymore!
            .expect(1)
            .mount(&mock_server)
            .await;
        // Act
